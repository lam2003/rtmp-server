 ...
 * @LastEditTime: 2020-03-18 13:26:16
 */
...
int FlvSegment::Initialize(Request* request)
{
    ...
    request_ = request;
    jitter_algorithm_ =
        (JitterAlgorithm)_config->GetDvrTimeJitter(request->vhost);
...
std::string FlvSegment::generate_path()
{
    ...
    flv_path = Utils::BuildStreamPath(flv_path, request_->vhost, request_->app,
                                      request_->stream);
...
int FlvSegment::on_update_duration(SharedPtrMessage* msg)
{
    ...
    if (stream_previous_pkt_time_ < 0 ||
...
int FlvSegment::UpdateFlvMetadata()
{
    ...
    if ((ret = writer_->Write(buf, AMF0_LEN_NUMBER, nullptr)) !=
        ERROR_SUCCESS) {
        return ret;
    }
...
int FlvSegment::WriteMetadata(SharedPtrMessage* shared_metadata)
{
    ...
    BufferManager manager;
    if ((ret = manager.Initialize(metadata->payload, metadata->size)) !=
        ERROR_SUCCESS) {
        return ret;
    }
    ...
    duration_offset_ =
        writer_->Tellg() + 11 + size - AMF0_LEN_OBJ_EOF - AMF0_LEN_NUMBER;
    // 2B string flag
    filesize_offset_ = duration_offset_ - AMF0_LEN_NUMBER -
                       AMF0_LEN_STR(std::string("duration"));
...
int FlvSegment::WriteVideo(SharedPtrMessage* shared_video)
{
    ...
    bool is_keyframe        = flv::Demuxer::IsAVC(payload, size) &&
                       flv::Demuxer::IsKeyFrame(payload, size) &&
                       !is_sequence_header;
...
int DvrSegmentPlan::update_duration(SharedPtrMessage* msg)
{
    ...
    if (_config->GetDvrWaitKeyFrame(request_->vhost)) {
        if (msg->IsVideo()) {
            ...
            int   size    = msg->size;
            bool  is_keyframe =
                flv::Demuxer::IsAVC(payload, size) &&
                flv::Demuxer::IsKeyFrame(payload, size) &&
                !flv::Demuxer::IsAVCSequenceHeader(payload, size);
            if (!is_keyframe) {
                return ret;
            }
            audio_num_before_segment_ = 0;
        ...
        if (audio_num_before_segment_ < RTMP_NUM_TO_JUDGE_DVR_ONLY_HAS_AUDIO &&
            msg->IsAudio()) {
...
int DvrSegmentPlan::OnAudio(SharedPtrMessage* shared_audio)
{
    ...
    if (flv::Demuxer::IsAACSequenceHeader(shared_audio->payload,
                                          shared_audio->size)) {
        rs_freep(sh_audio_);
...
int DvrSegmentPlan::OnVideo(SharedPtrMessage* shared_video)
{
    ...
    if (flv::Demuxer::IsAVCSequenceHeader(shared_video->payload,
                                          shared_video->size)) {
        rs_freep(sh_video_);
