...
static void
vhost_resolve(std::string& vhost, std::string& app, std::string& param)
{
...
static std::string generate_stream_url(const std::string& vhost,
...
Protocol::Protocol(IProtocolReaderWriter* rw)
{
    ...
    auto_response_when_recv_ = false;
}
...
Protocol::~Protocol()
{
    ...
    }
    {
        ...
        for (it = manual_response_queue_.begin();
             it != manual_response_queue_.end();) {
            Packet* packet = *it;
            rs_freep(packet);
...
/**
 * when fmt = 0,message header contain [timestamp delta][payload length][message
 * type][stream id],11 bytes when fmt = 1,message header contain [timestamp
 * delta][payload length][message type],7 bytes when fmt = 2,message header
 * contain [timestamp delta],3 bytes when fmt = 3,message header is null,0 bytes
 */
int Protocol::ReadMessageHeader(ChunkStream* cs, char fmt)
{
    ...
    if (cs->msg_count == 0 && fmt != RTMP_FMT_TYPE0) {
        ...
        // 0x42             where: fmt=1, cid=2, protocol contorl user-control
        // message 0x00 0x00 0x00   where: timestamp=0 0x00 0x00 0x06   where:
        // payload_length=6 0x04             where: message_type=4(protocol
        // control user-control message) 0x00 0x06            where: event
        // Ping(0x06) 0x00 0x00 0x0d 0x0f  where: event data 4bytes ping
        // timestamp.
        if (cs->cid == RTMP_CID_PROTOCOL_CONTROL && fmt == RTMP_FMT_TYPE1) {
            ...
            rs_error("chunk stream is fresh. fmt_require=%d, actual=%d, "
                     "cid=%d, ret=%d",
                     RTMP_FMT_TYPE0, fmt, cs->cid, ret);
            return ret;
        }
    }
    ...
    if (cs->msg && fmt == RTMP_FMT_TYPE0) {
        ret = ERROR_RTMP_CHUNK_START;
        rs_error(
            "chunk stream exists. fmt_require=%d, actual=%d, cid=%d, ret=%d",
            RTMP_FMT_TYPE0, fmt, cs->cid, ret);
        return ret;
    }
    ...
    if (fmt <= RTMP_FMT_TYPE2) {
        ...
        cs->extended_timestamp =
            (cs->header.timestamp_delta >= RTMP_EXTENDED_TIMESTAMP);
        ...
        if (fmt <= RTMP_FMT_TYPE1) {
            ...
            if (!is_first_msg_of_chunk &&
                cs->header.payload_length != payload_length) {
                ret = ERROR_RTMP_CHUNK_START;
                rs_error("msg exists in chunk cache. old size=%d, ret=%d",
                         cs->header.payload_length, ret);
                return ret;
            }
    ...
    if (cs->extended_timestamp) {
        ...
        /**
         * example 1:
         * (first_packet,without extended ts,ts = 0) --> (second_packet,with
         * extended ts,exts=40) is ok example 2: (first_packet,without extended
         * ts,ts = 0) --> (second_packet,without extended ts,ts=40) -->
         * (third_packet,with extended ts,exts=40)
         */
        if (!is_first_msg_of_chunk && chunk_timestamp > 0 &&
...
int Protocol::ReadMessagePayload(ChunkStream* cs, CommonMessage** pmsg)
{
    ...
    if (cs->header.payload_length <= 0) {
        rs_warn("get an empty rtmp messge(type=%d, size=%d, time=%lld, sid=%d)",
    ...
    memcpy(cs->msg->payload + cs->msg->size,
           in_buffer_->ReadSlice(payload_size), payload_size);
...
int Protocol::DoDecodeMessage(MessageHeader& header,
                              BufferManager* manager,
                              Packet**       ppacket)
{
    int ret = ERROR_SUCCESS;
    ...
    if (header.IsAMF3Command() || header.IsAMF3Data()) {
        ...
        if (command == RTMP_AMF0_COMMAND_RESULT ||
            ...
            if ((ret = AMF0ReadNumber(manager, transaction_id)) !=
                ERROR_SUCCESS) {
        ...
        else if (command == RTMP_AMF0_COMMAND_ON_METADATA ||
                 command == RTMP_AMF0_COMMAND_SET_DATAFRAME) {
...
int Protocol::DoSimpleSend(MessageHeader* header, char* payload, int size)
{
    ...
    while (p < end) {
        int nbh = 0;
        if (p == payload) {
            nbh = ChunkHeaderC0(header->perfer_cid, header->timestamp,
                                header->payload_length, header->message_type,
                                header->stream_id, c0c3);
        }
        else {
...
int Protocol::DecodeMessage(CommonMessage* msg, Packet** ppacket)
{
    ...
    *ppacket = nullptr;
    ...
    Packet* packet = nullptr;
    if ((ret = DoDecodeMessage(msg->header, &manager, &packet)) !=
        ERROR_SUCCESS) {
...
int Protocol::ManualResponseFlush()
{
    ...
    std::vector<Packet*>::iterator it;
    for (it = manual_response_queue_.begin();
         it != manual_response_queue_.end(); it++) {
        Packet* packet = *it;
    ...
    switch (msg->header.message_type) {
        ...
        case RTMP_MSG_WINDOW_ACK_SIZE:
            if ((ret = DecodeMessage(msg, &packet)) != ERROR_SUCCESS) {
                rs_error("decode packet from message payload failed. ret=%d",
                         ret);
                return ret;
            }
            break;
    ...
    switch (msg->header.message_type) {
        case RTMP_MSG_SET_CHUNK_SIZE: {
            ...
            if (pkt->chunk_size < RTMP_MIN_CHUNK_SIZE ||
                pkt->chunk_size > RTMP_MAX_CHUNK_SIZE) {
                rs_warn("accept chunk size:%d", pkt->chunk_size);
                if (pkt->chunk_size < RTMP_MIN_CHUNK_SIZE) {
                    ret = ERROR_RTMP_CHUNK_START;
                    rs_error("chunk size should be %d+, value=%d, ret=%d",
                             RTMP_MIN_CHUNK_SIZE, pkt->chunk_size, ret);
                    return ret;
                }
            }
